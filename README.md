...
[![Ably + Azure Cognitive Services Live Captioning Demo with LED display](http://img.youtube.com/vi/9loZY1q08tc/0.jpg)](https://www.youtube.com/watch?v=9loZY1q08tc)
...
This is a web app that uses [Azure Cognitive Service](https://azure.microsoft.com/services/cognitive-services/?WT.mc_id=aiml-11852-ayyonet) (ACS) and [Ably](https://www.ably.io/). It is designed to be run in a browser on your mobile phone while you're wearing mask with a flexible LED display. It uses ACS to perform streaming transcription of your microphone input (ideally from a lapel or headset mic, but will also work with the phone held within a sensible distance), while using the app in a browser. The data returned from ACS is then sent to the display on the mask via Ably. The transcription is also displayed in the browser.
...
For local development, you can use the [Azure functions SDK](https://docs.microsoft.com/azure/azure-functions/functions-develop-local?WT.mc_id=aiml-11852-ayyonet) to replicate this. For production, use static files (or files created by a static site generator of your choice) and Azure will serve them for you.

## The Azure function

The `/API` folder in this repo contains an [Azure functions JavaScript API](https://docs.microsoft.com/azure/azure-functions/create-first-function-vs-code-node?WT.mc_id=aiml-0000-ayyonet). It contains some files created by default (package.json, host.json etc) that you don't really need to worry about, they are generated by the Azure Functions SDK. If you wanted to expand the API, you would use npm install and the package.json file to manage dependencies for any additional functionality.
...
- Sign up for free Azure account if you don't already have a subscription.
